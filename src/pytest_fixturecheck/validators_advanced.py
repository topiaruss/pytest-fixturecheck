            ...
            if "__" in prop_path:
                ...
                for i, segment in enumerate(segments):
                    if not hasattr(current, segment):
                        ...
                        if strict: raise AttributeError(error_msg)
                        else: warnings.warn(error_msg); break
                    if i == len(segments) - 1:
                ...
                if not hasattr(obj, prop_path):
                    ...
                    if strict: raise AttributeError(error_msg)
                    else: warnings.warn(error_msg); continue
            ...
            if hasattr(typing, "get_origin") and hasattr(typing, "get_args"): # Handle Union types
                ...
                if origin is typing.Union:
                    ...
                    if not any(isinstance(actual_value, t) for t in type_args):
                        ...
                        if strict: raise TypeError(error_msg)
                        else: warnings.warn(error_msg)
                    continue 
